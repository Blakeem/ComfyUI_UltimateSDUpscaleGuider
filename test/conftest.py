...
def _find_comfyui_root() -> Path:
    ...
    Checks in order:
    1. COMFYUI_ROOT environment variable
    2. Standard layout (two directories up from repo root)
    """
    ...
    if env_root:
        ...
        if (path / "comfy").exists():
            return path
        raise ValueError(f"COMFYUI_ROOT={env_root} does not contain 'comfy' directory")

    # Standard layout: custom_nodes is inside ComfyUI
    standard_root = REPO_ROOT.parent.parent.resolve()
    if (standard_root / "comfy").exists():
        return standard_root

    raise ValueError(
        f"Could not find ComfyUI root. Checked:\n"
        f"  - COMFYUI_ROOT env var (not set)\n"
        f"  - Standard layout: {standard_root} (no 'comfy' directory found)\n"
        f"Set COMFYUI_ROOT environment variable to your ComfyUI installation path."
    )
...
def pytest_configure(config):
    ...
    # Assumes the name of the custom node folder is ComfyUI_UltimateSDUpscaleGuider
    args.whitelist_custom_nodes = ["ComfyUI_UltimateSDUpscaleGuider"]

    # Apply custom paths (loads extra_model_paths.yaml)
